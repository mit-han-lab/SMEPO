...
def compute_policy_loss_vanilla(
    ...
    is_teacher: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute the clipped policy objective and related metrics for PPO.
    ...
    Args:
        ...
        rollout_is_weights: `(torch.Tensor)`:
            IS weights for rollout correction, shape (batch_size, response_length).
        is_teacher: `(torch.Tensor)`:
            Mask indicating teacher trajectories (1) vs student (0), shape (batch_size,).
            Teacher trajectories use pure policy gradient (no IS), student uses standard PPO.
    """
    ...
    # Standard PPO loss for student trajectories
    student_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)

    # Handle mixed teacher/student training
    if is_teacher is not None:
        teacher_loss_weight = config.get("teacher_loss_weight", 1.0)
        teacher_loss_gamma = config.get("teacher_loss_gamma", 0.1)
        teacher_loss_mode = config.get("teacher_loss_mode", "rl_simple")

        if teacher_loss_mode == "sft":
            # SFT mode: use negative log likelihood directly (ignores advantages)
            teacher_losses = -log_prob * teacher_loss_weight
        elif teacher_loss_mode == "rl":
            teacher_losses = -ratio / (ratio + teacher_loss_gamma) * advantages * teacher_loss_weight
        else:
            # RL mode: pure policy gradient (no IS, since old_log_prob=0)
            teacher_losses = pg_losses1 * teacher_loss_weight

        # Expand is_teacher to match response_mask shape: (batch_size,) -> (batch_size, response_length)
        is_teacher_expanded = is_teacher.unsqueeze(-1).expand_as(response_mask)

        # Combine: use teacher_losses where is_teacher=1, student_losses where is_teacher=0
        pg_losses = torch.where(is_teacher_expanded > 0.5, teacher_losses, student_losses)
    else:
        pg_losses = student_losses

    # Apply rollout correction weights if provided (only for student trajectories)
    if rollout_is_weights is not None:
        if is_teacher is not None:
            # Only apply IS weights to student trajectories
            is_teacher_expanded = is_teacher.unsqueeze(-1).expand_as(response_mask)
            rollout_is_weights = torch.where(
                is_teacher_expanded > 0.5, torch.ones_like(rollout_is_weights), rollout_is_weights
            )
    ...
    if is_teacher is not None:
        is_teacher_expanded = is_teacher.unsqueeze(-1).expand_as(response_mask)
        student_mask = (1 - is_teacher_expanded) * response_mask
        teacher_mask = is_teacher_expanded * response_mask
        if student_mask.sum() > 0:
            pg_metrics["actor/student_loss"] = verl_F.masked_mean(student_losses, student_mask).detach().item()
        else:
            pg_metrics["actor/student_loss"] = 0.0
        if teacher_mask.sum() > 0:
            pg_metrics["actor/teacher_loss"] = verl_F.masked_mean(teacher_losses, teacher_mask).detach().item()
            pg_metrics["actor/teacher_ratio"] = teacher_mask.sum().item() / (teacher_mask.sum() + student_mask.sum()).item()
        else:
            pg_metrics["actor/teacher_loss"] = 0.0
            pg_metrics["actor/teacher_ratio"] = 0.0
