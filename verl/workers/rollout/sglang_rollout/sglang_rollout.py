...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            val_response_length = prompts.meta_info.get("response_length", None)
            val_params = {
            ...
            if val_response_length is not None:
                val_params["max_new_tokens"] = val_response_length
            request_sampling_params.update(val_params)
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        if not do_sample:
            ...
            val_response_length = req.meta_info.get("response_length", None)
            val_params = {
                "top_k": self.config.val_kwargs.top_k,
                "top_p": self.config.val_kwargs.top_p,
                "temperature": self.config.val_kwargs.temperature,
...
class ServerAdapter(BaseRollout):
    ...
    async def wake_up(self):
        """Wake up the rollout engine by resuming memory occupation."""
        await self.resume(tags=["kv_cache", "weights"])
        if self.device_mesh["infer_tp"].get_local_rank() == 0 and self.config.free_cache_engine:
            await self._engine.flush_cache()

    async def sleep(self):
        """Put the rollout engine to sleep by releasing memory occupation."""
        await self.release()
