...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        if getattr(config, "return_embeddings", False):
            emb_name = config.embedding_model_name if config.embedding_model_name else "BAAI/bge-small-en-v1.5"
            emb_device = getattr(config, "embedding_device", "cuda")
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for input in vllm_inputs:
            ...
            if i > 2:
                break
            ids = input["prompt_token_ids"]
            text = kwargs["tokenizer"].decode(ids, skip_special_tokens=False)
            print("@" * 100)
            print("@text@: ", text)
            print("@" * 100)'''
        ...
        if getattr(self.config, "return_embeddings", False) and self.embedder is not None:
            resp_texts = []
            resp_token_counts = []
            ds_texts = []
            for i in range(batch_size):
                valid = response_attention_mask[i].bool()
                valid_ids = response[i][valid]
                resp_token_counts.append(int(valid_ids.numel()))
                text = self.tokenizer.decode(valid_ids.tolist(), skip_special_tokens=True)
                resp_texts.append(text)
                if not prompts.meta_info["is_eval"]:
                    ds_texts.append(prompts[i].non_tensor_batch["extra_info"]["deepseek_reasoning"])
            ...
            if not prompts.meta_info["is_eval"]:
                ...
                cos = (emb * ds_emb).sum(axis=1)

                non_tensor_batch["response_text"] = np.array(resp_texts, dtype=object)
                non_tensor_batch["response_embedding"] = emb
                non_tensor_batch["response_token_count"] = np.array(resp_token_counts, dtype=np.int32)
                non_tensor_batch["cos_sim"] = cos
                print(f"####################### cos_sim: {cos} #######################")
        ...
        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch, meta_info={"eos_token_id": eos_token_id})
