...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self.cos_sim_ema = self.config.actor.cos_sim_ema
        self.beta = self.config.actor.beta
        self.sign = self.config.actor.sign
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            try:
                x = data.non_tensor_batch["cos_sim"].mean().item()
                self.cos_sim_ema = self.beta * self.cos_sim_ema + (1 - self.beta) * x
                lr = base_lr * (1 + 0.3 * self.sign * self.cos_sim_ema)
                metrics["actor/cos_sim"] = x
            except:
                lr = base_lr
                metrics["actor/cos_sim"] = 0.0
