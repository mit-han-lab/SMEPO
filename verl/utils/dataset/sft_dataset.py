...
class SFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | ListConfig, tokenizer, config, max_samples: int = -1):
        ...
        self.use_verl_mix_response = config.get("use_verl_mix_response", False)
        self.teacher_model_name = config.get("teacher_model_name", None)
        self.create_val_from_train = config.get("create_val_from_train", False)
    ...
    def _read_files_and_tokenize(self):
        ...
        if not self.use_verl_mix_response:
            ...
            if isinstance(self.prompts, pd.DataFrame):
                self.prompts = self.prompts.squeeze()
            self.prompts = [x[1]['content'] if x[0]['role'] == "system" else x[0]['content'] for x in self.prompts]
        if not self.use_verl_mix_response:
            ...
            self.responses = self.dataframe["extra_info"]
            if isinstance(self.responses, pd.DataFrame):
                self.responses = self.responses.squeeze()
            self.responses = self.responses.tolist()
            response_list = []
            ...
            for i in range(len(self.responses)):
                """if self.teacher_model_name in self.responses[i]:
                    prompt_responses = self.responses[i][self.teacher_model_name]
                    if prompt_responses is not None:
                        best_response = prompt_responses[0]["text"]
                        best_reward = prompt_responses[0]["reward"]
                        for response in prompt_responses:
                            if response["reward"] > best_reward:
                                best_response = response["text"]
                                best_reward = response["reward"]
                        if best_reward == 1:
                            best_response_think = "<think>\n" + best_response if ("</think>" in best_response and not "<think>" in best_response) else best_response
                            response_list.append(best_response_think)
                            keep_indices.append(i)"""
                keep_indices.append(i)
                response_list.append(self.responses[i]["response"])
            
            print(f"keep {len(keep_indices)} responses out of {len(self.responses)}")
            self.responses = response_list
            self.prompts = [self.prompts[i] for i in keep_indices]
            print(f"{self.prompts[0]=}")
            print(f"{self.responses[0]=}")
